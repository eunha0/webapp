...
✅ **참고 자료(제시문) 첨부**: 과제 프롬프트에 최대 11개의 제시문 첨부 가능 (4개 기본 + 7개 추가)  
✅ **제시문 이미지 업로드 기능** (NEW! 🎉): 제시문 작성 시 이미지 업로드 및 자동 텍스트 추출
  - **이미지 OCR**: Google Cloud Vision API 사용한 한글/영어 텍스트 추출
  - **즉시 텍스트 변환**: 업로드된 이미지에서 텍스트를 자동 추출하여 입력 필드에 자동 삽입
  - **Cloudflare R2 저장**: 업로드된 이미지는 R2 스토리지에 영구 보관
✅ **파일 업로드 기능** (학생 답안용): 이미지(JPG, PNG) 및 PDF 파일 업로드 후 자동 텍스트 추출
  - **이미지 OCR**: Google Cloud Vision API 사용한 한글/영어 텍스트 추출
  - **PDF 처리**: 텍스트 기반 PDF는 PDF.js로 추출, 이미지 기반 PDF는 OCR 처리
  - **실시간 처리**: 파일 업로드 시 즉시 텍스트 추출 및 처리 진행  
✅ **채점 기록**: 과거 채점 세션 및 결과 저장 및 검토  
✅ **과제 출력 기능** (NEW! 🎉): 과제 상세 정보를 출력 가능 형태로 제공
  - **과제 정보 출력**: 과제 제목, 설명, 제시문, 평가 루브릭, 학생 액세스 코드 포함
  - **인쇄 최적화**: 깔끔한 인쇄 레이아웃으로 종이 출력 가능
  - **스크롤 가능 미리보기**: 새 창에서 전체 내용 확인 후 출력  
...
2. **과제 정보 입력**:
   ...
   - 마감일: 날짜 선택 (선택사항)
3. **제시문 추가** (NEW! 🎉):
   - 4개 기본 제시문 입력 필드 제공 (최대 11개까지 추가 가능)
   - 각 제시문에 텍스트 직접 입력 또는 이미지 업로드
   - **이미지 업로드 기능**:
     - "이미지 업로드" 버튼 클릭
     - 이미지 파일 선택 (JPG, PNG, WebP, 최대 10MB)
     - 자동 OCR 처리로 텍스트 추출
     - 추출된 텍스트가 입력 필드에 자동 삽입
     - 처리 시간: 5-10초
4. **루브릭 기준 설정**:
   - 기본 4개 기준 자동 추가
   - "평가 기준 추가" 버튼으로 추가 가능
...
#### 과제 상세 정보 확인 및 출력 (NEW! 🎉)

1. **과제 카드 클릭**: 과제 상세 정보 모달 열기
2. **표시되는 정보**:
   - 과제 제목 및 설명
   - 제시문 (입력된 경우 모두 표시)
   - 평가 루브릭 (기준 및 설명)
...
3. **출력 기능**:
   ...
   - 새 창에서 과제 전체 정보 표시
   - 내용 확인 후 "출력" 버튼으로 인쇄
   - 포함 항목: 과제 제목, 설명, 제시문, 평가 루브릭, 학생 액세스 코드
4. **삭제 버튼**: 과제 삭제 (학생 제출물도 함께 삭제됨)
...
2. **6자리 숫자 코드 자동 생성** (예: 123456)
3. **학생들에게 코드 공유 방법**:
   - **화면 공유**: 과제 상세 모달에서 직접 코드 표시
   - **인쇄물 배포**: "출력" 버튼으로 액세스 코드 포함하여 인쇄
   - **디지털 공유**: 코드 복사하여 이메일/메신저로 전송
...
4. **일괄 출력 기능 사용**:
   - 각 답안 왼쪽 체크박스로 출력할 항목 선택
   - "전체 선택" 체크박스로 모든 답안 선택
   - "출력" 버튼 클릭하여 드롭다운 메뉴 열기
   - **PDF (개별 출력)**: 각 답안을 별도 창에서 열어 개별 인쇄
...
✅ **논술 과제 생성 및 관리**  
  - 과제 제목, 설명, 학년 수준, 마감일  
  - 제시문 입력 (최대 11개, 텍스트 또는 이미지 업로드)  
  - 제시문 이미지 업로드 및 자동 OCR 처리 (NEW! 🎉)
  - 맞춤형 루브릭 기준 추가 (동적 UI)  
✅ **과제 상세 정보 출력** (NEW! 🎉)
  - 과제 정보 전체 인쇄 (제목, 설명, 제시문, 루브릭, 액세스 코드)
  - 스크롤 가능한 인쇄 미리보기
  - 인쇄 최적화 레이아웃  
✅ **학생 답안지 업로드 및 관리**  
  - 학생 이름 및 논술 내용 입력  
...
✅ **CMS 시스템 (Admin 페이지)**  
✅ **평가 관련 자료 관리**  
  - 루브릭 자료 (10개 플랫폼 루브릭 세부 내용 게시)
  - 논술 평가 자료  
  - 기출 문제 (PDF 파일)
✅ **드롭다운 네비게이션 메뉴**
✅ **플랫폼 루브릭 10개 완전 구현** (NEW! 🎉 - 2025-11-28):
  - **기본 루브릭 (3개)**:
    - 표준 논술 루브릭 (4개 기준)
    - 상세 논술 루브릭 (6개 기준)
    - 간단 논술 루브릭 (3개 기준)
  - **뉴욕 주 루브릭 (4개, 4점 만점 기준)**:
    - 뉴욕 주 리젠트 시험 논증적 글쓰기 루브릭
    - 뉴욕 주 리젠트 시험 분석적 글쓰기 루브릭
    - 뉴욕 주 중학교 논술 루브릭
    - 뉴욕 주 초등학교 논술 루브릭
  - **IB 중등 프로그램 루브릭 (3개, 4점 만점 기준)**:
    - IB 중등 프로그램 고등학교 개인과 사회 논술 루브릭
    - IB 중등 프로그램 중학교 개인과 사회 논술 루브릭
    - IB 중등 프로그램 과학 논술 루브릭
  - 모든 루브릭 기준 데이터 JSON 형식으로 구현
  - 과제 생성 시 드롭다운에서 선택 가능
  - '평가 관련 자료' > '루브릭' 페이지에 세부 내용 게시
  - 교사가 루브릭 내용을 사전에 확인 가능
✅ **루브릭 목록 UI 개선** (NEW! 🎉 - 2025-11-28):
  - 제목 한 번만 표시 (중복 제거)
  - 왼쪽 정렬로 깔끔한 레이아웃
  - 기출 문제 페이지와 동일한 디자인 적용
  - 검색 기능 정상 작동  
...
### 일괄 출력 및 내보내기 (NEW! 🎉 - 2025-11-09 구현 완료)

✅ **채점 이력 체크박스 시스템**
  - **개별 선택**: 각 답안지 왼쪽에 체크박스 표시
  - **전체 선택/해제**: 상단 체크박스로 모든 항목 일괄 선택
  - **선택 카운터**: 현재 선택된 항목 수 실시간 표시
  - **시각적 피드백**: 선택 상태에 따른 버튼 활성화/비활성화

✅ **출력 드롭다운 메뉴**
  - **PDF (개별 출력)**:
    - 선택한 각 답안지를 별도 창에서 열기
    - 개별 인쇄/저장 가능
    - 순차적 창 열기 (500ms 간격)
  - **단일 PDF 파일로 내보내기**:
    - 선택한 모든 답안지를 하나의 문서로 통합
    - 답안지별 페이지 구분 (page-break)
    - 상단에 전체 개수 표시
    - 일괄 인쇄 버튼 제공

✅ **내보내기 기능**
  - **완전한 정보 포함**: 학생 답안, 점수, 피드백 모두 포함
  - **인쇄 최적화 레이아웃**: 깔끔한 페이지 구성
  - **반응형 디자인**: 프린터 설정에 맞춰 자동 조정
  - **에러 핸들링**: 개별 답안 로드 실패 시 계속 진행

✅ **채점 이력에서 재검토 기능** (NEW! 🎉)
  - **클릭 가능한 답안 카드**: 마우스 호버 시 시각적 피드백
  - **체크박스 분리**: 체크박스 클릭은 선택만, 카드 클릭은 재검토
  - **빠른 접근**: "클릭하여 재검토 →" 버튼 표시
  - **자동 데이터 로드**: 제출물 정보와 피드백 자동 조회
  - **검토 모달 재사용**: 동일한 Split-Screen 모달 사용
  - **수정 및 저장**: 피드백 수정 후 데이터베이스 업데이트
  - **출력 가능**: 재검토 모달에서 직접 출력
...
- **Major Updates**:
  ...
  - ✅ **제시문 이미지 업로드 기능 구현** (2025-11-27):
    - 과제 생성 시 제시문에 이미지 업로드 버튼 추가
    - 이미지 업로드 후 자동 OCR 처리
    - 추출된 텍스트를 제시문 입력 필드에 자동 삽입
    - 이미지 미리보기 및 업로드 진행 상태 표시
    - Cloudflare R2에 영구 저장
  - ✅ **과제 출력 기능 구현** (2025-11-27):
    - 과제 상세 모달에 "출력" 버튼 추가
    - 과제 정보 전체 출력 (제목, 설명, 제시문, 루브릭, 액세스 코드)
    - 스크롤 가능한 인쇄 미리보기 (새 창)
    - 인쇄 최적화 레이아웃
  - ✅ **채점 결과 검토 및 출력 기능** (2025-11-09 오전):
    - Split-Screen 검토 모달 (학생 답안 좌측, 피드백 우측)
    - 실시간 피드백 편집 기능
    - 채점 결과 출력 기능 (인쇄 최적화 레이아웃)
    - 제출물 상세 조회 API 추가
  - ✅ **일괄 출력 및 내보내기 기능** (2025-11-09 오후):
    - 채점 이력 체크박스 선택 시스템
    - 전체 선택/해제 기능
    - 출력 드롭다운 메뉴 (PDF 개별/통합 출력)
    - 선택한 답안지 일괄 인쇄 기능
    - 단일 PDF 통합 문서 생성
  - ✅ **채점 이력 재검토 기능** (2025-11-09 오후):
    - 답안 카드 클릭 시 검토 모달 열기
    - 제출물 및 피드백 자동 로드
    - Split-Screen 모달 재사용
    - 피드백 수정 및 저장 기능
  - ✅ **플랫폼 루브릭 10개 전체 구현** (2025-11-28):
    - 10개 플랫폼 루브릭 기준 데이터 완전 구현
    - 표준/상세/간단 논술 루브릭 (3개)
